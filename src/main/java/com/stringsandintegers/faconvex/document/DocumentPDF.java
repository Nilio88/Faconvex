...
class DocumentPDF implements Document {
    ...
    public void addOwnerMessage(Message message) throws IOException {
        ...
        //Print the sender and the date of the message on the PDF document (right-aligned)
        content.beginText();
        ...
        //Print the message on the PDF document (right-aligned)
        content.beginText();
        content.setFont(messageFont, messageFontSize);
        content.moveTextPositionByAmount(-messageWidth, 0);
        content.showText(message.getMessage());
        content.moveTextPositionByAmount(messageWidth, 0);
        content.newLine();
        content.endText();
    }
    ...
    public void addRecipientMessage(Message message) throws IOException {
        ...
        //Print the sender and the date of the message on the PDF document (left-aligned)
        content.beginText();
        content.setFont(messageFont, messageFontSize);
        content.showText(outMessage);
        content.newLine();
